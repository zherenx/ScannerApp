...
The Scanner App collects data using sensors on an iOS device. The Scanner App has three modes, single camera, dual camera and RGB-D. We are also looking to get the stereo disparity estimation result from Apple's api, but this feature is currently in progress.
...
User can navigate to respective recording scene by tapping Camera, DualCam or LiDARDepth button on the main menu.
...
Each scan has a unique identifier in the format of <yyyyMMdd'T'HHmmssZ>_<device_id>. Files of a scan use the scan id as filename and they have different file extensions.

* metadata (```<scanId>.json```)
* RGB video (```<scanId>.mp4```)
  * note that dual camera mode videos are named ```<scanId>-primary.mp4``` and ```<scanId>-secondary.mp4```
* motion data files
...
For dual camera mode, on an iPad pro 11-inch (2nd generation), output videos are at 1440 x 1920 resolution and 60 FPS for the primary camera (wide angle camera), 1080 x 1920 resolution and 60 FPS for the secondary camera (ultrawide camera).
...
DualCameraRecordingManager also uses the AVFoundation framework, but it has a AVCaptureMultiCamSession and connections need to be set manully.
